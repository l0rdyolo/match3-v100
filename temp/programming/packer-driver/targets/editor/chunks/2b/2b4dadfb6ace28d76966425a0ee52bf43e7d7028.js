    ...
    execute: function () {
      ...
      _export("GridManager", GridManager = (_dec = ccclass("GridManager"), _dec(_class = class GridManager extends (_crd && SingletonComponent === void 0 ? (_reportPossibleCrUseOfSingletonComponent({
        ...
        async handleSelection(pieceA, pieceB) {
          ...
          const [_pieceA, _pieceB] = await this.SwapPieces(pieceA, pieceB);
          console.log(_pieceA, _pieceB);
          return;
          let matches = await this.matchChecker.checkForMatches(_pieceA, _pieceB, this.grid);

          if (matches.length > 0) {
            await this.deleteMatches(matches);
            await this.sleep(300);
            await this.gravityHandler.applyGravity(this.grid);
            await this.sleep(1000); // await this.fillEmptySpaces();

            await this.sleep(900); //buraya bir sleep patlar
...
//# sourceMappingURL=2b4dadfb6ace28d76966425a0ee52bf43e7d7028.js.map