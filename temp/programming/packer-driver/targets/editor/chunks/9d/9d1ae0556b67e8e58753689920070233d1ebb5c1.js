      ...
      _export("GravityHandler", GravityHandler = (_dec = ccclass('GravityHandler'), _dec2 = property(_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
        ...
        applyGravityToCol(columnPieceCount, matches) {
          for (const [col, count] of columnPieceCount) {
            ...
            let highestRow = (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
              error: Error()
            }), GridManager) : GridManager).getInstance().gridHeight - 1; // Aşağıya indirilecek parçaları bulun

            for (let i = 0; i < (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
              error: Error()
            }), GridManager) : GridManager).getInstance().gridHeight; i++) {
              const piece = (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                error: Error()
              ...
              if (piece) {
                ...
                if (newRow < (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                  ...
                  (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                    error: Error()
                  }), GridManager) : GridManager).getInstance().grid[newRow][col] = piece;
                  piece.row = newRow;
                  piece.updatePosition(); // Eski pozisyonu boşalt

                  (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                    error: Error()
                  }), GridManager) : GridManager).getInstance().grid[i][col] = null;
                }
              }
            }
          }
...
//# sourceMappingURL=9d1ae0556b67e8e58753689920070233d1ebb5c1.js.map