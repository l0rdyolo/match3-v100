      ...
      _export("GravityHandler", GravityHandler = (_dec = ccclass('GravityHandler'), _dec2 = property(_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
        ...
        applyGravityToCol(columnPieceCount, matches) {
          for (var [col, count] of columnPieceCount) {
            console.log(col + " s\xFCtununu " + count + " birim a\u015Fa\u011F\u0131 indirin.");
            console.log(); // Bu sütundaki en yüksek satırı bul (matched row'lar)

            var highestRow = (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
              error: Error()
            }), GridManager) : GridManager).getInstance().gridHeight - 1; // Aşağıya indirilecek parçaları bulun

            for (var i = 0; i < (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
              error: Error()
            }), GridManager) : GridManager).getInstance().gridHeight; i++) {
              var piece = (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                error: Error()
              }), GridManager) : GridManager).getInstance().grid[i][col];
            ...
            for (var _i = highestRow; _i >= 0; _i--) {
              var _piece = (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                error: Error()
              }), GridManager) : GridManager).getInstance().grid[_i][col];

              if (_piece) {
                var newRow = _i + count;

                if (newRow < (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                  error: Error()
                }), GridManager) : GridManager).getInstance().gridHeight) {
                  // Parçayı yeni pozisyona taşı
                  (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                    error: Error()
                  }), GridManager) : GridManager).getInstance().grid[newRow][col] = _piece;
                  _piece.row = newRow;
                  ...
                  (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                    error: Error()
                  }), GridManager) : GridManager).getInstance().grid[_i][col] = null;
                }
              }
            }
          }
...
//# sourceMappingURL=1c328f3bd43c8892d68453b0d7185d82b4b9a071.js.map