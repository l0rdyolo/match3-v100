          ...
          return _asyncToGenerator(function* () {
            if (_this.isSelectionValid()) {
              ...
              if (matches.length > 0) {
                ...
                console.log("önce geldi"); // Gravity sonrası yeni eşleşmeleri kontrol et

                do {
                  matches = yield _this.matchChecker.checkForMatchesAfterGravity();

                  if (matches.length > 0) {
                    (_crd && GridManager === void 0 ? (_reportPossibleCrUseOfGridManager({
                      error: Error()
                    }), GridManager) : GridManager).getInstance().deleteMatches(matches);
                    yield _this.gravityHandler.applyGravity(); // Yeni gravity işlemini uygula ve bekle
                  }
                } while (matches.length > 0);
              } else {
                // Eğer eşleşme yoksa, parçaları eski konumlarına geri kaydır
                yield _this.sliderManager.Slide(_this.secondSelected, _this.firstSelected);
...
//# sourceMappingURL=3cc79e7e17b1d40e02cf0a78809dad28f8ba25e7.js.map