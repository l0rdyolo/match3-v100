        ...
        generate(level) {
          ...
          for (var row = rows - 1; row >= 0; row--) {
            for (var col = cols - 1; col >= 0; col--) {
              var currentClass = gridData[col][rows];
              ...
              if (assetType && assetType.length > 0) {
                ...
                var randomAsset = assetType[0];

                if (randomAsset) {
                  var newNode = instantiate(randomAsset);
                  this.node.addChild(newNode);
                  var position = this.getTilePosition(rows - row, cols - col, tileSize);
...
//# sourceMappingURL=ef25de063d942b0ccb6d383e68548bee0ee7e0e8.js.map