      ...
      _export("GridManager", GridManager = (_dec = ccclass("GridManager"), _dec(_class = class GridManager extends (_crd && SingletonComponent === void 0 ? (_reportPossibleCrUseOfSingletonComponent({
        ...
        handleSelection(pieceA, pieceB) {
          ...
          return _asyncToGenerator(function* () {
            ...
            if (matches.length > 0) {
              ...
              yield _this2.fillEmptySpaces();
              var newMatches = yield _this2.matchChecker.checkForMatchesAfterGravity(_this2.grid); // const newMatches = await this.matchChecker.checkForMatchesAfterGravity(
              //   this.grid
              // );
        ...
        logMatchesByGroups(matches) {
          ...
          for (var piece of matches) {
            ...
            var group = [piece];
            visited.add(piece); // Bu parça ile aynı olan diğer parçaları bul

            for (var otherPiece of matches) {
              if (!visited.has(otherPiece) && this.arePiecesConnected(piece, otherPiece)) {
          ...
          groupedMatches.forEach((group, index) => {
            console.log("Grup " + (index + 1) + ":", group.map(p => p.node.name));
          });
        }

        arePiecesConnected(piece1, piece2) {
          // Örneğin, burada basit bir mesafe kontrolü yapıyoruz
          var rowDiff = Math.abs(piece1.row - piece2.row);
          var colDiff = Math.abs(piece1.col - piece2.col);
...
//# sourceMappingURL=ebc01ca5c344058dad4156436e15f4ecb7526eef.js.map