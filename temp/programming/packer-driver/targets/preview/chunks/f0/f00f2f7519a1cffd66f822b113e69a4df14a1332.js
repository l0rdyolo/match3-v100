  ...
  return {
    ...
    execute: function () {
      ...
      _export("GravityHandler", GravityHandler = (_dec = ccclass('GravityHandler'), _dec(_class = class GravityHandler extends (_crd && SingletonComponent === void 0 ? (_reportPossibleCrUseOfSingletonComponent({
              ...
              for (var col = 0; col < _this.grid[0].length; col++) {
                for (var row = _this.grid.length - 1; row > 0; row--) {
                  if (_this.grid[row][col] === null && _this.grid[row - 1][col] !== null) {
                    yield _this.movePieceDown(row - 1, col);
                    moved = true;
                  }
                }
              }
        ...
        movePieceDown(row, col) {
          var _this2 = this;

          return _asyncToGenerator(function* () {
            var piece = _this2.grid[row][col];

            if (piece) {
              var targetRow = _this2.findLowestEmptyRow(col, row);

              if (targetRow !== row) {
                _this2.grid[targetRow][col] = piece;
                _this2.grid[row][col] = null;
                piece.row = targetRow;
        ...
        findLowestEmptyRow(col, startRow) {
          for (var row = this.grid.length - 1; row > startRow; row--) {
...
//# sourceMappingURL=f00f2f7519a1cffd66f822b113e69a4df14a1332.js.map