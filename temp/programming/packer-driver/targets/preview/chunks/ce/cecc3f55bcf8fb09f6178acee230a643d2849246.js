    ...
    execute: function () {
      ...
      _export("GridGenerator", GridGenerator = (_dec = ccclass('GridGenerator'), _dec2 = property(_crd && PiecesPool === void 0 ? (_reportPossibleCrUseOfPiecesPool({
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var row = 0; row < _this.GRID_ROW; row++) {
              ...
              var _loop = function _loop(col) {
                ...
                piece.setPosition(pieceFirstPosition); // piece.on(Node.EventType.TOUCH_START, this.onPieceClicked, this);

                piece.on(Node.EventType.TOUCH_START, event => {
                  _this.interactionManager.onPieceClicked(event, piece);
                });
                grid[row][col] = piece;
                piecesInRow.push(piece); // await EffectManager.fallPieceToGridPosition(piece , pieceLastPosition , 0.05 ,  10)
              };
              ...
              yield _this.animateRowFall(piecesInRow, 0.1);
            }
          })();
        }

        animateRowFall(piecesInRow, delay) {
          return _asyncToGenerator(function* () {
            var promises = piecesInRow.map(piece => {
              var targetPosition = piece.getPosition().clone();
              return (_crd && EffectManager === void 0 ? (_reportPossibleCrUseOfEffectManager({
                error: Error()
              }), EffectManager) : EffectManager).fallPieceToGridPosition(piece, targetPosition, delay, 10);
            });
        ...
        getCenteredPosition(j, i) {
          ...
          var _j = j * this.PIECE_CONTENT_SIZE - this.PIECE_CONTENT_SIZE * Math.floor(this.GRID_ROW / 2);

          var _i = i * this.PIECE_CONTENT_SIZE - this.PIECE_CONTENT_SIZE * Math.floor(this.GRID_ROW / 2);
...
//# sourceMappingURL=cecc3f55bcf8fb09f6178acee230a643d2849246.js.map