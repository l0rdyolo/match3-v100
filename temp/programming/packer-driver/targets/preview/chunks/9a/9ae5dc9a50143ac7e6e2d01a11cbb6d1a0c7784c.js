    ...
    execute: function () {
      ...
      _export("GridManager", GridManager = (_dec = ccclass("GridManager"), _dec(_class = class GridManager extends (_crd && SingletonComponent === void 0 ? (_reportPossibleCrUseOfSingletonComponent({
        ...
        handleSelection(pieceA, pieceB) {
          ...
          return _asyncToGenerator(function* () {
            yield _this.sliderManager.SwapPieces(pieceA, pieceB);
            var isMatch = false;
            var matches = yield _this.matchChecker.checkForMatches(pieceA, pieceB, _this._grid);
            ...
            if (matches.length === 0) {
              ...
              isMatch = true;
              yield _this.deleteMatches(matches);
            }

            while (isMatch) {
              yield _this.sleep(300);
              yield _this.gravityHandler.applyGravity(_this._grid);
              yield _this.sleep(400);
              yield _this.fillEmptySpaces();
              yield _this.sleep(400);
              isMatch = yield _this.matchChecker.checkForMatchesAfterGravity(_this._grid);
            }
          })();
        ...
        fillEmptySpaces() {
          ...
          return _asyncToGenerator(function* () {
            ...
            for (var row = 0; row < _this2.gridHeight; row++) {
              var promises = []; // Her satır için bir promises dizisi oluştur

              for (var col = 0; col < _this2.gridWidth; col++) {
                ...
                if (piece.isEmpty) {
                  ...
                  newPieceNode.setParent(_this2.node);

                  _this2.node.addChild(newPieceNode); // newPieceNode.getComponentInChildren(Sprite).color =
                  //   this.colors.yellow;
                  ...
                  piece.node.setPosition(new Vec3(piece.col * ((_crd && GameGlobal === void 0 ? (_reportPossibleCrUseOfGameGlobal({
                    error: Error()
                  }), GameGlobal) : GameGlobal).PIECE_CONTENT_SIZE + (_crd && GameGlobal === void 0 ? (_reportPossibleCrUseOfGameGlobal({
                    error: Error()
                  }), GameGlobal) : GameGlobal).PIECE_OFFSET), _this2.gridWidth * (_crd && GameGlobal === void 0 ? (_reportPossibleCrUseOfGameGlobal({
                    error: Error()
                  }), GameGlobal) : GameGlobal).PIECE_CONTENT_SIZE, 0));
                  promises.unshift(piece.updatePosition(row, col)); // Promises dizisine ekle
                }
              }

              yield Promise.all(promises); // Tüm promises'lerin tamamlanmasını bekle
            }
          })();
        }
...
//# sourceMappingURL=9ae5dc9a50143ac7e6e2d01a11cbb6d1a0c7784c.js.map