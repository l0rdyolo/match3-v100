  ...
  return {
    ...
    execute: function () {
      ...
      _export("GridGenerator", GridGenerator = (_dec = ccclass("GridGenerator"), _dec2 = property(CCInteger), _dec3 = property(CCInteger), _dec(_class = (_class2 = class GridGenerator extends Component {
        ...
        start() {//!TODO: grid yerleşmesini ve content size'ı responsive yapmalıyız
        ...
        createsMatchByName(name, row, col, grid) {
          var _grid$col, _grid$col2, _grid$row, _grid$row2;

          var isMatch = false;

          if (row >= 2 && ((_grid$col = grid[row - 1][col]) == null ? void 0 : _grid$col.node.name) === name && ((_grid$col2 = grid[row - 2][col]) == null ? void 0 : _grid$col2.node.name) === name) {
            isMatch = true;
          }

          if (col >= 2 && ((_grid$row = grid[row][col - 1]) == null ? void 0 : _grid$row.node.name) === name && ((_grid$row2 = grid[row][col - 2]) == null ? void 0 : _grid$row2.node.name) === name) {
            isMatch = true;
          }
...
//# sourceMappingURL=6bc3e25d315e6a4e938a16c2477cf53cb5bb5d5d.js.map