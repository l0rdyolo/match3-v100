        ...
        handleSelection(pieceA, pieceB) {
          ...
          return _asyncToGenerator(function* () {
            ...
            if (matches.length > 0) {
              ...
              yield _this2.fillEmptySpaces(); //buraya bir sleep patlar

              var newMatches = yield _this2.matchChecker.checkForMatchesAfterGravity(_this2.grid);
              yield _this2.sleep(1000); // 1 saniye bekl

              while (newMatches.length > 0) {
                // Döngü ile kontrolü tekrar yapıyoruz
                yield _this2.deleteMatches(newMatches);
                yield _this2.gravityHandler.applyGravity(_this2.grid);
                yield _this2.fillEmptySpaces();
                yield _this2.sleep(1000); // 1 saniye bekl

                newMatches = yield _this2.matchChecker.checkForMatchesAfterGravity(_this2.grid);
              }
            } else {
              yield _this2.SwapPieces(pieceA, pieceB);
            }
...
//# sourceMappingURL=1e0c36591b1f133484022ebb369dcb84e531842e.js.map